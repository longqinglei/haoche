...
+(void)showSubVehicleSuccess:(UIView*)view{
    //get window size and position
    CGFloat width = HCSCREEN_WIDTH*0.45;
    CGFloat height = HCSCREEN_WIDTH*0.21;
    CGRect windowRect = [[UIScreen mainScreen] bounds];
    ...
    //set background color
    //if a background image is used, use the image instead.
    alertView.backgroundColor = UIColorFromRGBValue(0x000000);
    ...
    //and custom color
    UILabel *titleLabel = [[UILabel alloc] init];
    titleLabel.text = @"已成功提醒";
    titleLabel.textAlignment = NSTextAlignmentCenter;
    titleLabel.textColor = UIColorFromRGBValue(0xffffff);
    ...
    imageView.image = [UIImage imageNamed:@"subsuccess"];
    imageView.frame = CGRectMake(0, 0, 18, 18);
    imageView.frame = CGRectMake((width-18-titleLabel.width-5)/2, 15, 18, 18);
    titleLabel.frame = CGRectMake(imageView.right+5, 15, titleLabel.width, 18);
    [alertView addSubview:imageView];
    UILabel *bottomLabel = [[UILabel alloc] init];
    bottomLabel.text = @"可在我的-上新提醒中查看";
    bottomLabel.textAlignment = NSTextAlignmentCenter;
    bottomLabel.textColor = UIColorFromRGBValue(0x9f9f9f);
    ...
    //CGSize bottomLabelSize = [titleLabel sizeThatFits:CGSizeMake(width - kInsetValue, height - kInsetValue)];
    bottomLabel.frame = CGRectMake(0,height-30, width,15);
    [alertView addSubview:bottomLabel];
    //check wether the alert is of custom type or not
    //if it is, set the custom view
//    UIView *content = type == FVAlertTypeCustom ? contentView : [self contentViewFromType:type];
//    
